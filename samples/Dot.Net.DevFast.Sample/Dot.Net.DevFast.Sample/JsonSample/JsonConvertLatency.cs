...
namespace Dot.Net.DevFast.Sample.JsonSample
{
    public static class JsonConvertLatency
    {
        ...
        private static double MeasureJsonConvert(this object val, int iteration, bool print = true)
        {
        ...
        private static double MeasureDevFast(this object val, StringBuilder sb, int iteration, bool print = true)
        {
            var json = string.Empty;
            var sw = Stopwatch.StartNew();
            for (var i = 0; i < iteration; i++)
            {
                val.ToJson(sb, new JsonSerializer());
                json = sb.ToString();
                sb.Clear();
            }
            sw.Stop();
